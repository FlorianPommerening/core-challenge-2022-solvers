...
SearchStatus SymbolicSearch::step() {
    step_num++;
    int previous_num_plans = solution_registry->get_num_found_plans();
...
void SymbolicSearch::print_info(int previous_num_of_plans) {
    if (step_num > 0) {
        if (!lower_bound_increased ||
            (silent && previous_num_of_plans == solution_registry->get_num_found_plans())) {
            return;
        }
    }
...
void SymbolicSearch::setLowerBound(int lower) {
    if (lower < 0) {
        utils::g_log << "Overflow of lower cost bound to " << lower << "!" << endl;
...
void SymbolicSearch::add_options_to_parser(OptionParser &parser) {
    ...
    parser.add_option<bool>("simple", "simple/loopless plan construction",
                            "false");
    parser.add_option<bool>("silent", "only print steps where new plans have been found",
                            "false");
}
} // namespace symbolic
